...
__global__ void ReLU_GPU(float *input,
		float *output,
		int rows,
		int cols) {
...
__global__ void ReLU_derivative_GPU(float *input,
				    float *output,
				    int rows,
				    int cols) {
    // Calculate row and column indices
...
__global__ void update_parameters_GPU_old(float *W1, float *b1,
				      float *W2, float *b2,
...
__global__ void calculate_accuracy_GPU(float* Y, float* Y_pred, int rows, float* accuracy) {
    ...
    // Calculate the accuracy
    *accuracy = ((float)correct_count) / rows;
}
...
// Make Predictions
    ...
    // Initialize float d_accuracy on device
    float *d_accuracy;
    cudaMalloc((void **)&d_accuracy, sizeof(float));
